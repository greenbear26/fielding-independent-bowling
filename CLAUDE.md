...
aggregation, and fitting the regression — it ends once the coefficients are printed.
...
Stages 1–3 are the notebook, which is a linear chain of in-memory DataFrames and must be run top to
bottom. Stage 4 is `fib.py`, which is independent — it reads `data/bowler-stats.csv` from disk, so
...
2. **Aggregate per bowler-season** (cell 11) → `bowler_df`, written to `data/bowler-stats.csv`
   Nested loop over `Year` × `Bowler`. **Bowler-seasons under 100 legal balls are skipped.**
3. **Fit** (cells 13–15) → `fib_reg`
   Fits a `LinearRegression` on 2015+ seasons, regressing per-over feature rates against `Economy`.
   Cell 14 plots predicted vs actual; cell 15 prints the coefficients. **The notebook ends here** —
   copy the printed coefficients into `fib.py` to score them.
4. **Score** — `fib.py`, run standalone or imported.

### Scoring a coefficient vector (`fib.py`)

`evaluate_fib(coefficients)` applies a vector, normalizes, pairs each season with the bowler's
following season, and prints `corr(FIB, next_year_economy)` against
...
output of cell 15, not a hand-tuned divergence from it; it's the starting point for tuning.
...
| `bowler-stats.csv` | Generated by cell 11, all years from 2009, 1072 rows | `Year, Bowler` |
...
- The notebook's aggregation (cell 11) uses explicit Python loops (`iterrows`, nested `for`) rather
